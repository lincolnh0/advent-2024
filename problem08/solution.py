...
def solution_p1(input_data):
    distinct_channels = Counter(
        [y for x in input_data.splitlines() for y in x if y != "."]
    )
    ...
    antennas_coordinates = {
        channel: (
            [
                ...
                for x in re.finditer(channel, input_data)
            ]
        )
    ...
    for channel, antennas in antennas_coordinates.items():
        for first_antenna in antennas:
            for second_antenna in [x for x in antennas if first_antenna != x]:
                ...
                vector_2 = (
                    second_antenna[0] - first_antenna[0],
                    second_antenna[1] - first_antenna[1],
                )

                antinodes = (
                    antinodes
                    | {(first_antenna[0] + vector_1[0], first_antenna[1] + vector_1[1])}
                    | {
                        (
                            second_antenna[0] + vector_2[0],
                            second_antenna[1] + vector_2[1],
                        )
                    }
                )
    ...
    return len(
        {
            x
            for x in antinodes
            if 0 <= x[0] < len(roof_map) and 0 <= x[1] < len(roof_map)
        }
    )
...
def solution_p2(input_data):
    ...
    antinodes = {
        y for coordinates in antennas_coordinates.values() for y in coordinates
    }
        ...
        for first_antenna in antennas:
            for second_antenna in [x for x in antennas if first_antenna != x]:
                ...
                antinodes = (
                    ...
                    | {
                        (
                            first_antenna[0] + vector_1[0] * x,
                            first_antenna[1] + vector_1[1] * x,
                        )
                        for x in range(1, len(roof_map))
                    }
                    | {
                        (
                            second_antenna[0] + vector_2[0] * x,
                            second_antenna[1] + vector_2[1] * x,
                        )
                        for x in range(1, len(roof_map))
                    }
                )
