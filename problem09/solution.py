...
def solution_p1(input_data):
    file_blocks_lengths = {
        int(x / 2): int(input_data[x]) for x in range(0, len(input_data), 2)
    }
    ...
    spaces = [
        y
        for start, length in space_blocks.items()
    ...
    for file_id in file_blocks_start:
        checksum += sum(
            [
                file_id * x
                for x in range(
                    file_blocks_start[file_id],
                    min(
                        total_block_length,
                        file_blocks_start[file_id] + file_blocks_lengths[file_id],
                    ),
                )
            ]
        )
    ...
    checksum += sum(
        [
            file_id * index
            for file_id, index in list(zip(list(reverse_file_blocks), spaces))
            if index < total_block_length
        ]
    )
...
def solution_p2(input_data):
    ...
    for file_id, block_length in reversed(file_blocks_lengths.items()):
        for space_index, space_length in sorted(space_blocks.items()):
            if (
                block_length <= space_length
                and space_index < file_blocks_start[file_id]
            ):
                file_blocks_start[file_id] = space_index
                space_blocks.pop(space_index)
                if space_length - block_length > 0:
                    space_blocks[space_index + block_length] = (
